...
export default function CartMenu({ cart }: { cart?: Omit<Cart, "beforeInsert" | "afterLoad"> | null }) {
    ...
    return (
        <div className={`fixed inset-0 z-30 ${cartState ? "animate-slide-in-cart" : "animate-slide-out-cart translate-x-full"}`}>
            <div className={`absolute inset-0 z-30 bg-curtain transition-opacity duration-500 ease-in-out delay-[0.35s] ${cartState ? "opacity-25": "opacity-0"}`} onClick={cartMenuClose} />
