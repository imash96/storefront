...
export default function MobileMenuClient({ children, regions }: { regions: Region[] | null } & React.PropsWithChildren) {
    ...
    return (
        <div className={`fixed inset-0 z-30 lg:hidden ${menuState ? "animate-slide-in-menu " : "animate-slide-out-menu -translate-x-full"}`}>
            ...
            <div className="absolute inset-0 z-40 flex w-11/12 sm:w-5/6 max-w-md border-r border-grey-11">
                <div className="relative flex w-full flex-col shadow-xl bg-scroll bg-naviBg text-grey-33 bg-grey-20">
                    <div className="flex justify-between px-4 py-3 sticky top-0 border-b border-grey-17 z-10 bg-grey-18">
                    ...
                    {children}
                    <div className="flex justify-between px-4 py-3 sticky bottom-0 border-t border-grey-17 z-10 bg-grey-18">
                        {/* Account */}
