...
export async function createCart(data = {}) {
    ...
    return medusaClient.carts.create(data, headers)
        .then(({ cart }) => cart)
        .catch((err) => {
            console.log(err)
            return null
...
export async function addShippingAddress(
    ...
    return medusaClient.customers.addresses.addAddress(data, headers).then(({ customer }) => customer).catch((err) => medusaError(err))
}
