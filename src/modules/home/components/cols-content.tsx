    ...
    return (
        <div className="grid mt-6 gap-4 justify-items-center xs:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-6 lg:gap-x-8 xl:gap-4">
            {filteredcollection.map((collection) => (
                <a key={collection.id} href={collection.title} className="relative shadow-md shadow-grey-84 flex h-80 min-w-72 xs:min-w-40 xs:min-h-64 sm:min-w-72 md:min-w-56 md:min-h-80 xm:min-w-72 lg:min-w-56 xl:min-w-48 flex-col overflow-clip rounded-lg p-6 hover:opacity-75 xl:w-auto" >
