...
const SwiperConfig: SwiperProps = {
    className: 'review-swiper w-full h-full',
    speed: 900,
    spaceBetween: 10,
    slidesPerView: 1,
    loop: true,
            ...
            <Swiper {...SwiperConfig}>
                {slides.map((slide) => (
                    <SwiperSlide key={slide.id} className="!flex flex-col items-center h-auto text-center">
                        <div className="slider_text relative mb-7 text-gray-82 text-sm p-5 bg-grey-19 border-2 border-grey-17 rounded-md flex-grow basis-32">
                            {slide.para}
                        </div>
                        <Image src={slide.imageUrl} alt={slide.title} width={96} height={96} className="mb-5 object-cover w-24 h-24 rounded-full" />
                        <h3 className="mb-1 font-bold text-xl">{slide.title}</h3>
                        <div className="mb-1 text-xs text-grey-83">{slide.href}</div>
