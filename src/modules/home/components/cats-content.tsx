                ...
                <div key={xIndex} className="grid grid-cols-1 gap-4 mt-6 md:grid-cols-3 md:grid-rows-2 lg:gap-6">
                    {categories.map((category, yIndex) => (
                        <div key={category.id} className={`group aspect-w-2 aspect-h-1 shadow-md shadow-grey-84 overflow-hidden rounded-md md:h-full ${(0 == xIndex && 0 == yIndex || 1 == xIndex && yIndex == 1) ? "md:col-span-2 md:row-span-2" : "md:aspect-none md:relative md:h-full"}`}>
                            ...
                            />
                            <div aria-hidden="true" className="bg-gradient-to-b from-transparent to-grey-23 opacity-50 md:absolute md:inset-0" />
                            <div className="flex items-end p-6 md:absolute md:inset-0">
                                <div>
                                    <h3 className="font-bold text-grey-81 text-lg">
                                        <LocalizedClientLink href={`/category/${category.handle}`}>
                                            <span className="absolute inset-0" />
                                            {category.name}
                                        </LocalizedClientLink>
                                    </h3>
                                    <p aria-hidden="true" className="mt-0.5 text-base text-grey-83 flex">
                                        Shop now <ArrowRight />
                                    </p>
                                </div>
                            </div>
